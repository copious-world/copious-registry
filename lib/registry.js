...
class Registry {
    ...
    sort_by_score(results) {   // <- an array
        if ( Array.isArray(results) ) {
            results = results.sort((a,b) => {
    ...
    // Order a set of record by a monotonic score... (they must include a 'score' numeric field)
    sort_by_fn(results,func) {   // <- an array <-- to parameter func(store type,store type)
        if ( typeof func === 'function' ) {
            if ( Array.isArray(results) ) {
                results = results.sort(func)
            } else {
                try {
                    let vals = results.values()
    ...
    add_just_one(f_obj,from_new) {
        ...
        if ( is_new ) {
            if ( Array.isArray(this.global_file_list) ) {
                ...
                f_obj.entry = this.unique_id(f_obj)
                f_obj.score = 1.0
                ...
                f_obj.score = 1.0
                f_obj.entry = this.global_file_list.unique_id(f_obj)
            ...
            if ( f_obj._tracking ) {
                ...
                if ( Array.isArray(f_obj) ) {
                    this.global_file_list.push(f_obj)
                    f_obj.entry = this.unique_id()
                } else {
                    f_obj.global_file_list.entry = this.unique_id()
                }
